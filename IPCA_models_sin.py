...
class IPCA(nn.Module):
    ...
    def forward(self, x, kernal, rd, polar_pos, att_mask,cross_att_mask, i_kernal):
...
class Block(nn.Module):
    ...
    def forward(self, x, kx, rd, polar_pos, att_mask, cross_att_mask): 
        b = x.shape[0]
        i_kx = repeat(self.kernel_token_i, '() k d -> b k d', b=b)
        x, kx, i_kx = self.norm1(x), self.norm1(kx), self.norm1_i(i_kx)
        x_, kx_ = self.attn(x, kx, rd, polar_pos, att_mask,cross_att_mask, i_kx) 
        x = x + x_
        kx = kx + kx_
...
class IpcaViT(nn.Module):
    ...
    def forward_encoder(self, x, wsi_rd, wsi_polar, token_mask, kernel_mask, kernel_tokens, i_kernel_mask, device, mask_ratio):
        # embed patches
        ...
        for blk in self.blocks:
            x, kernel_tokens = blk(x, kernel_tokens, wsi_rd_mask, wsi_polar_mask,
                                               att_mask=att_mask, cross_att_mask= cross_att_mask) 

        return x, kernel_tokens

    def forward(self, imgs, wsi_rd, wsi_polar, token_mask, kernel_mask, device, mask_ratio):
        b = imgs.shape[0]
        ...
        latent, kernel_tokens= self.forward_encoder(imgs, wsi_rd, wsi_polar, token_mask, kernel_mask,
                                                                     kernel_tokens, i_kernel_mask, device, mask_ratio)
